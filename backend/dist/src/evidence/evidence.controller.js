...
let EvidenceController = class EvidenceController {
    ...
    async upload(findingId, file, evidenceType, description, clientHash, user, req) {
        ...
        if (clientHash && clientHash !== result.sha256Hash) {
            return {
                ...result,
                hashMismatchWarning: 'Client hash differs from server hash — possible transit corruption',
            };
        }
        return result;
    }
    async supersede(evidenceId, file, reason, user, req) {
        if (!file)
            throw new common_1.BadRequestException('No file uploaded');
        if (!reason)
            throw new common_1.BadRequestException('Supersession reason is required');
    ...
    async verifyIntegrity(evidenceId, user) {
